...
def check(news: Optional[BrainSummary], market: Optional[BrainSummary],
          chart: Optional[BrainSummary], *, strict_mode: bool = True) -> dict:
    ...
    if all(p == "allow" for p in perms):
        ...
        if strict_mode:
            # 3-of-3 required: every brain must produce a clear direction
            # AND they must all agree.
            if len(clear) == 3 and len(set(clear)) == 1:
                dir_word = "buy" if clear[0] == "bullish" else "sell"
                return {"status": "aligned", "direction": dir_word,
                        "details": f"all_allow_{clear[0]}_3of3_strict"}
            if len(set(clear)) > 1:
                return {"status": "conflicting", "direction": "none",
                        "details": f"directions={directions}_strict"}
            # 2-of-3 with one neutral or unclear -> conflicting in strict
            return {"status": "conflicting", "direction": "none",
                    "details": f"directions={directions}_lacks_3of3"}
        # Loose mode (legacy): 2-of-3 same direction is enough
