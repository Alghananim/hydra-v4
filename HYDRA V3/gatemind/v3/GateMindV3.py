...
class GateMindV3:
    def __init__(self, *, strict_mode: bool = True):
        # Strict mode (Phase 2): B grades block, alignment requires 3-of-3.
        # Default True - the safe production posture for HYDRA V3.
    ...
    def decide(self, *, pair: str,
        ...
        with sw.stage("alignment_check"):
            try:
                ali = alignment.check(news, market, chart, strict_mode=self.strict_mode)
        ...
        with sw.stage("daily_limits_check"):
            try:
                # FIX #2B: pass configurable consecutive_losses_limit so the
                # gate honours ValidationConfig instead of a hardcoded 3.
                cl_limit = (state.consecutive_losses_limit
                            if state is not None else 3)
                sc = state_check.check(state, now,
                                       consecutive_losses_limit=cl_limit)
