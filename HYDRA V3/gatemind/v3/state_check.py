...
def check(state: Optional[SystemState], now_utc: datetime,
          consecutive_losses_limit: int = 3) -> dict:
    """FIX #2B: consecutive_losses_limit is now an explicit parameter so
    the gate honours ValidationConfig.consecutive_losses_limit (default 2)
    instead of hardcoding 3. Default kept at 3 for backward compatibility
    ...
    # Cooldown after consecutive losses (limit is now configurable)
    if state.consecutive_losses >= consecutive_losses_limit:
        ...
        return {"position_state": f"after_{consecutive_losses_limit}_losses_cooldown",
                "daily_limits": "ok",
                "details": f"{state.consecutive_losses}>={consecutive_losses_limit}_consecutive_losses_no_cooldown_set"}
