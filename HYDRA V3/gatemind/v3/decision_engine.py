...
def synthesize(decision: GateDecision,
               *, news: Optional[BrainSummary],
               ...
               strict_mode: bool = True) -> GateDecision:
    ...
    # ---------------------------------------------------------------- HARD WAITS
    waiting = []
    # Strict mode (Phase 2): B grades become BLOCKING (no half-permission trades).
    if strict_mode:
        if news is not None and news.grade == "B": blocking.append("news_grade_B_strict")
        if market is not None and market.grade == "B": blocking.append("market_grade_B_strict")
        if chart is not None and chart.grade == "B": blocking.append("chart_grade_B_strict")
    else:
