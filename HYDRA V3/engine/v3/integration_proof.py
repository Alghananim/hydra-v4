...
# Import all 5 brains
from newsmind.v3 import NewsMindV3, NewsItem
from newsmind.v3.sources import NewsSource
...
def run_scenario(name, *, news_headline, news_age_min=1, market_slope, chart_slope,
    ...
    nm = NewsMindV3(pair="EUR/USD", calendar=None, sources=sources)
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    result = engine.decide_and_maybe_trade(
        ...
        chart_assessment=ca,
        recent_bars=make_chart_bars(80, slope=0.00012, noise=0.000005, seed=5),
        spread_pips=0.5, slippage_pips=0.3,
        now_utc=NOW_OUT)
    engine.nb.flush(timeout_s=2.0)
