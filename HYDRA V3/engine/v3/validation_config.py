...
STRICT_MODE_DEFAULT = True             # B grade => block; require 3-of-3 alignment

# FIX #3B: single source of truth for pair status. execution_check reads
# this so there is exactly ONE place to update pair status.
PAIR_STATUS = {
    "EUR/USD": "production",
    "USD/JPY": "monitoring",
    "GBP/USD": "disabled",
...
class ValidationConfig:
    ...
    pair_status: Dict[str, str] = field(default_factory=lambda: dict(PAIR_STATUS))
    ...
    def validate_or_die(self):
        ...
        if self.broker_env not in ("practice", "paper", "live"):
            errors.append(f"OANDA_ENV={self.broker_env} not in (practice, paper, live)")
        if errors:
            raise SystemExit(
