...
class Storage:
    ...
    # ---------------- Engine state (RAM-backed counters) ----------------
    def write_engine_state(self, key: str, value) -> bool:
        """Persist a single key/value pair (value is JSON-encoded)."""
        try:
            ...
            self.warnings.append(f"write_engine_state_failed:{key}:{e}")
            return False

    def read_engine_state(self, key: str, default=None):
        """Read a previously stored value. Returns default if missing/error."""
        try:
            with _LOCK:
            ...
            self.warnings.append(f"read_engine_state_failed:{key}:{e}")
            return default
