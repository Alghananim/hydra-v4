...
def _build_user_prompt(brain_outputs: dict, gate_decision: dict) -> str:
    ...
    # FIX #1: pull the actual pair from brain_outputs; default safely to EUR/USD
    # if absent so older callers still work. Without this, every USD/JPY review
    # was hallucinated about EUR/USD.
    pair = brain_outputs.get("pair", "EUR/USD")
    return f"""Review this trading decision for {pair}.
...
def review_brain_outputs(brain_outputs: dict,
    ...
    _http_call: OPTIONAL dependency-injected function for tests. Signature:
        (system: str, user: str) -> tuple[Optional[str], str]
        i.e. same shape as OpenAIClient.chat. Tests pass a capturing mock.
    ...
    """
    if _http_call is None and not LLM_AVAILABLE:
        return LLMReview(error=LLM_DISABLED_REASON or "llm_unavailable")

    client = _get_client() if _http_call is None else None
    user_prompt = _build_user_prompt(brain_outputs, gate_decision)

    t0 = time.time()
    if _http_call is not None:
        content, err = _http_call(_SYSTEM_PROMPT, user_prompt)
    ...
    if err:
        return LLMReview(error=err, model_used=model_used, latency_ms=latency_ms)

    # Parse JSON response
    try:
        parsed = json.loads(content) if content else {}
    except (json.JSONDecodeError, TypeError):
        return LLMReview(error="bad_json", model_used=model_used,
